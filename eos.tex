...
With the trace anomalies in hand, the energy density, pressure and entropy density are easily interrelated,
\begin{eqnarray}
...
This completes the specification of the equations of state used in the analysis. We now briefly describe the hybrid model simulation into which the equations of state are embedded.

\section{Hybrid Model}

The equations of state are compared using the VISHNU transport model which couples the boost invariant viscous fluid dynamics code VISH2+1 \cite{?} for the hot and dense early 
phase of the collision to UrQMD for modeling the microscopic kinetic interactions of hadronic scattering and freeze-out \cite{?}. The mock particle data generated by each 
simulated event are then stored and analyzed to measure the spectra, flow and Bertsch-Pratt radii.
