...
\end{eqnarray}
where the endpoints $T_1 \equiv T_\text{sw}$ and $T_2 \equiv 180$ MeV smoothly interpolate the lattice results into the UrQMD trace anomaly at the desired switching 
...
$T_\text{sw} \equiv T_\text{c} = 154$ MeV.

 
While lower switching temperatures permit a smoother transition between the lattice and UrQMD equations of state, they 
necessitate chemical non-equillibirum corrections which occur naturally in UrQMD but are not accounted for in the present lattice parameterizations \cite{?}. The modified 
interaction measures, labeled with a prime to distinguish them from the raw lattice results, are plotted in Fig.~\ref{fig:trace_final}. The vertical gray line 
...
the implementation of the model used in this analysis; for a more detailed explanation of the VISHNU model, we refer the reader to \cite{}. 
...
provide a reasonable description of measured spectra and flows for the system at hand. For the purposes of this study, we assume vanishing bulk viscosity, although 
it would be interesting to account for finite bulk viscosity in future work, as its functional form is sensitive to the peak of the QCD trace anomaly near the QGP 
phase transition \cite{?}.

In order to switch from hydrodyamic field equations to microscopic transport, the model asserts a sudden switching temperature $T_\text{sw}$ at which the 
...
We choose a hydro-to-micro switching temperature $T_\text{sw} = 154$ MeV.

 which coincides with the pseudo-critical phase transition temperature of the
the HotQCD equation of state

 temperature where the lattice equations of state is constructed to match 
