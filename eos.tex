...
the peak of the QCD trace anomaly near the QGP phase transition \cite{Karsch:2007jc}. Unfortunately, bulk viscous corrections do not have a straight forward implementation in the present hybrid model 
...
in centrality bins $0$--$5\%$, $20$--$30\%$ and $40$--$50\%$. The ratio plot in the figure subset shows that the spread in the particle yield for the HotQCD errors is $\mathcal{O}(5\%)$ across all centralities. 
This spread is significantly smaller than the $\mathcal{O}(20 \%)$ deviations observed between the different EoS in Fig.~\ref{fig:spectra}. The elliptic flow $v_2$ shown in Fig.~\ref{fig:v2} and triangular flow $v_3$ in Fig.~\ref{fig:v3}
show similarly small sensitivies to the HotQCD errors with variations of $\mathcal{O}(5\%)$ for $v_2$ and $\mathcal{O}(10\%)$ for $v_3$. This establishes that the error in the HotQCD calculation leads to minor variations of hydrodynamic
observables.
...
two state of the art calculations by the HotQCD and Wuppertal-Budapest collaborations as well the s95p-v1 parameterization based on older lattice data. We find that the particle spectra calculated using the latest HotQCD and WB EoS
are indistinguishable within the statistical errors of our analysis, while the s95p-v1 EoS is substantially softer and generates spectra which deviate by as much as $30\%$ within the applicability of the model. No significant dependence
was observed for the different particle species or different centrality classes.
Similar sensitivity was observed in the elliptic flow harmonics $v_2$ and $v_3$ calculated on the Cooper-Frye freezeout hypersurface. The elliptic flow produced by the HotQCD and WB EoS was essentially identical, while the softer s95p-v1
parameterization produced $\mathcal{O}(10\%)$ less elliptic flow across centralities. The triangular flow $v_3$ was slightly more sensitive to changes in the hydrodynamic EoS and revealed a small $\mathcal{O}(5\%)$ discrepancy between the HotQCD
and WB EoS, and much larger 10--25\% discrepancies with the s95p-v1 EoS which were largest in peripheral centralities. We also calculate Bertsch-Pratt radii for the three different EOS, but were not able to detect meaningful differences in $R_\text{o}$,
$R_\text{s}$ and $R_\text{l}$ within the errors of our analysis. We note that a recent Bayesian sensitivity study observed stronger sensitivity of the Bertsch-Pratt radii to differences in the hydrodynamic equation of state  
